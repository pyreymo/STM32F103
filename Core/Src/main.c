...
int main(void)
{
  ...
  while (1)
  {
    ...
    int32_t hum_int, hum_frac;
    int32_t temp_int, temp_frac;
    float_to_int_parts(dht.Humidity, &hum_int, &hum_frac);
    float_to_int_parts(dht.Temperature, &temp_int, &temp_frac);

    ssd1306_Fill(Black);

    OLED_Printf(0, 0, Font_11x18, White, "System Info");
    OLED_Printf(0, 20, Font_7x10, White, "Hum:  %ld.%02ld %%", hum_int, hum_frac);
